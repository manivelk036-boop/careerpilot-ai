...
import { Clock, CheckCircle2, XCircle, ArrowRight, Trophy, Zap, Award, Target } from 'lucide-react';
  ...
  quant: [
    {
      ...
      correct: 0,
      explanation: 'Speed = 60 km/h = 60 * (5/18) = 50/3 m/s. Time = Distance / Speed = 150 / (50/3) = 9 seconds.'
    },
    {
      id: 2,
      ...
      correct: 2,
      explanation: 'Let the number be x. 0.20 * x = 80 => x = 400. 25% of 400 = 100.'
    },
    {
      id: 3,
      question: 'A shopkeeper marks his goods 30% above cost price and gives 10% discount. What is the net profit %?',
      options: ['17%', '18%', '19%', '20%'],
      correct: 0,
      explanation: 'Let CP = 100. Marked Price = 130. Discount = 10% of 130 = 13. Selling Price = 130 - 13 = 117. Profit = 17%.'
    },
    {
      id: 4,
      question: 'Simple interest on a sum at 5% per annum for 3 years is ₹3600. Find the principal sum.',
    ...
    },
    {
      ...
      correct: 0,
      explanation: 'Let numbers be 3x and 5x. (3x + 10) / (5x + 10) = 5/7 => 21x + 70 = 25x + 50 => 4x = 20 => x = 5. Numbers are 15 and 25.'
    }
  ],
  logical: [
    {
      id: 1,
      question: 'In a family, there are six members A, B, C, D, E and F. A and B are a married couple, A being the male member. D is the only son of C, who is the brother of A. E is the sister of D. How is E related to C?',
      options: ['Sister', 'Daughter', 'Cousin', 'Mother'],
      correct: 1,
      explanation: 'D is the son of C and E is the sister of D. Thus, E is the daughter of C.'
    },
    {
      id: 2,
      ...
      correct: 1,
      explanation: "Since the speaker has no brother or sister, 'my father's son' is himself. So, the man in the photograph's father is the speaker himself. Thus, it is his son's photograph."
    },
    {
      id: 3,
      question: 'If in a certain code language, MONKEY is coded as XDJMNL, how is TIGER coded in that code?',
      options: ['QDFHS', 'SDFHS', 'UJHFS', 'SHFDQ'],
      correct: 0,
      explanation: 'Each letter is shifted 1 position backward in reverse order. Y-1=X, E-1=D, K-1=J, N-1=M, O-1=N, M-1=L. Applying this to TIGER: R-1=Q, E-1=D, G-1=F, I-1=H, T-1=S. So, QDFHS.'
    },
    {
      id: 4,
      ...
      correct: 1,
      explanation: 'This is a geometric progression where each term is half of the previous term. (1/4) * (1/2) = 1/8.'
    },
    {
      id: 5,
      question: 'Find the odd one out: Geometry, Algebra, Calculus, Thermodynamics',
      options: ['Geometry', 'Algebra', 'Calculus', 'Thermodynamics'],
      correct: 3,
      explanation: 'Geometry, Algebra, and Calculus are branches of pure mathematics, while Thermodynamics is a branch of physics.'
    }
  ],
  verbal: [
    {
      id: 1,
      question: 'Choose the word that is most nearly synonymous with: IMPETUOUS',
      options: ['Calm', 'Rash', 'Diligent', 'Slow'],
      correct: 1,
      explanation: 'Impetuous means acting or done quickly and without thought or care; rash.'
    },
    {
      id: 2,
      question: 'Choose the word that is most nearly antonymous with: CAPRICIOUS',
      options: ['Fickle', 'Stable', 'Weak', 'Wild'],
      correct: 1,
      explanation: 'Capricious means given to sudden and unaccountable changes of mood or behavior. Stable is the opposite.'
    },
    {
      id: 3,
      question: 'Fill in the blank: The manager was _______ with the progress of the project.',
      options: ['satisfies', 'satisfying', 'satisfied', 'satisfy'],
      correct: 2,
      explanation: 'The past participle "satisfied" fits the passive construction "was satisfied with".'
    },
    {
      id: 4,
    ...
    },
    {
      ...
      options: ['To waste time', 'To work late into the night', 'To cook late', 'To light up a lamp'],
      correct: 1,
...
export default function Aptitude() {
  ...
  const handleNext = () => {
    const q = aptitudeQuestions[activeCategory][qIndex];
    ...
    if (qIndex + 1 < aptitudeQuestions[activeCategory].length) {
      setQIndex(i => i + 1);
    } else {
  ...
  const handleFinish = (ans = answers) => {
    ...
    const total = aptitudeQuestions[activeCategory].length;
    const score = Math.round((correct / total) * 100);
    setFinalScore(score);
    setShowResult(true);

    // Save score in student store
    const updatedAptitude = { ...aptitudeScores, [activeCategory]: score };
    updateProfile({ aptitudeScores: updatedAptitude });

    // Update overall placement breakdown for aptitude
    ...
    // Reward XP/Coins with level-up detection
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass p-8 text-center">
            ...
            <div className="grid grid-cols-3 gap-4 mt-8">
              ...
              <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30">
                <p className="text-2xl font-bold text-red-400">❌ {aptitudeQuestions[activeCategory].length - correct}</p>
  ...
  if (activeCategory) {
    const q = aptitudeQuestions[activeCategory][qIndex];
    const progress = (qIndex / aptitudeQuestions[activeCategory].length) * 100;
    const cat = categories.find(c => c.id === activeCategory);
    return (
      <Layout title={cat.title}>
        <div className="max-w-2xl mx-auto space-y-4">
          <div className="glass p-4">
            ...
            <p className="text-xs text-slate-500 mt-1.5">Question {qIndex + 1} of {aptitudeQuestions[activeCategory].length}</p>
          </div>
            ...
            <motion.div key={qIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="glass p-6">
              ...
              {selected !== null && (
                <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  {qIndex + 1 < aptitudeQuestions[activeCategory].length ? <><span>Next Question</span><ArrowRight size={16} /></> : <><span>View Results</span><Trophy size={16} /></>}
                </motion.button>
              )}
            </motion.div>
          </AnimatePresence>
